...
# QQ:                45646231
# Date:              2023-03-20
# FileName:          install_zabbix_server5.0.sh
# E-Mail:            lilei@xacdyun.com
# Version:           2.0.0
# Description:       install zabbix server 5.0 with Apache2 PHP7 MySQL
# Copyright (C) 2023 li lei
# OS DISTRIBUTION and OS VERSION:
# zabbix 5.0 LTS for RHEL\CentOS\OL\Rocky 7,8
# zabbix 5.0 LTS for Ubuntu 1804,2004
#======================================================================#
...
install_zabbix() {
    if [ $ID = "centos" -o $ID = "rocky" -o $ID = "ol" ]; then
        if [ ${VERSION_ID} == "7" -o ${VERSION_ID} == "8" ]; then
...
config_mysql_zabbix() {
    ...
    if [ $ID = "centos" -o $ID = "rocky" -o $ID = "ol" ]; then
        if [ ${VERSION_ID} == "8" ]; then
            ...
            sed -i -e "/.*date.timezone.*/c php_value[date.timezone] = Asia/Shanghai" -e "/.*upload_max_filesize.*/c php_value[upload_max_filesize] = 20M" /etc/opt/rh/rh-php72/php-fpm.d/zabbix.conf
