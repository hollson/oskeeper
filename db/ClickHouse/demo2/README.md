...
&nbsp; - `cpu\_metrics`: 存储CPU监控数据

&nbsp; - `net\_metrics`: 存储网络IO监控数据
...
\- `COLLECT\_INTERVAL`: 监控采集间隔（秒）

\- `BATCH\_SIZE`: 批量写入阈值

\- `DB\_FILE\_PATH`: 数据库文件路径

\- `CPU\_TABLE\_NAME`: CPU监控表名

\- `NET\_TABLE\_NAME`: 网络监控表名
...
\- `cpu\_percent`: 整体 CPU 使用率

\- `cpu\_cores`: 各核心 CPU 使用率（数组）
