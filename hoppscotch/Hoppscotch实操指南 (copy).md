...
HOPP_AIO_ALTERNATE_PORT=3000

DATA_ENCRYPTION_KEY=J7vQx2nY8pLkR3mZ9sT4wE6fG1hJ5oD2
WHITELISTED_ORIGINS=http://192.168.3.31:3000/admin,http://192.168.3.31:3000,app://localhost_3200,app://hoppscotch

#-----------------------Frontend Config------------------------------#
# Base URLs
VITE_BASE_URL=http://192.168.3.31:3000
VITE_SHORTCODE_BASE_URL=http://192.168.3.31:3000
VITE_ADMIN_URL=http://192.168.3.31:3000/admin
...
# 只映射 HOPP_AIO_ALTERNATE_PORT 指定的8080端口（容器内8080映射到主机8080）
docker run -d \
  --name hoppscotch \
  -p 3000:3000 \
  --env-file .env \
...
1. **在初始化页面填写 SMTP 配置**：回到 `http://192.168.3.31:3100/onboarding` 的认证配置页，按以下参数填写（适配本地 SMTP）：
   ...
   | 发件人邮箱    | work@gmail.com       | 自定义局域网邮箱（无需真实存在）       |
   | 用户名 / 密码 | 123456               | 无需真实认证，填任意值即可             |
